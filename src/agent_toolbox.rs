    ...
    match tool {
        ...
        "Wait" => json!({
            ...
            "properties": {
                ...
                "final_answer": {"type": ["string", "null"], "description": "The complete answer when state is completed; otherwise null. The text is returned in full without a length limit or truncation."},
                ...
                "progress": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "assistant_text": {"type": "string", "description": "Complete Worker text emitted with this model step, returned in full without a length limit or truncation."},
...
fn worker_examples(tool: &str) -> &'static str {
    match tool {
        "Ask" => {
            r#"{"prompt":"Purpose: inventory the specified directory. Rules and boundaries: work only inside <path>; read only; do not create, modify, move, or delete anything; do not interpret the project or recommend changes. Work: return a directory tree. Required evidence: include every listed file's path, type, size, and modification time; report access errors exactly."}
{"prompt":"Purpose: obtain source evidence for Manager analysis. Rules and boundaries: inspect only <module>; read only; do not modify anything, infer missing code, diagnose, or propose changes. Work: list all function and method signatures with exact file paths and line numbers, then return the complete source around <target>, including nearby definitions required to read it. Required evidence: preserve source text exactly and report omissions or read errors."}
    ...
    match tool {
        ...
        "Wait" => json!({
            ...
            "properties": {
                ...
                "final_answer": {
                    "type": ["string", "null"],
                    "description": "The complete answer when state is completed; otherwise null. The text is returned in full without a length limit or truncation."
                },
                "error": {
                    "type": ["string", "null"],
                ...
                "progress": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "assistant_text": {
                                "type": "string",
                                "description": "Complete assistant text emitted with this model step, returned in full without a length limit or truncation."
                            },
                            "tool_calls": {
                                "type": "array",
                                "items": {"type": "string"},
...
fn instructions(tool: &str) -> &'static str {
    match tool {
        ...
        "Wait" => {
            "Wait observes the sub-Agent for at most max_wait_ms, which may range from 0 for an immediate check through 3600000 for a one-hour wait. Every result returns progress observed since the preceding Wait as complete assistant text paired with ordered tool names; it never includes tool arguments or results. pending means no observable progress occurred since the preceding Wait; it does not mean the task failed or stopped. working means progress occurred but no final answer exists yet. completed returns the complete final answer. interrupted means work stopped before producing a final answer. failed means work ended because of an error. If a real user follow-up arrives while waiting, Wait returns immediately with state=wait_interrupted and reason=follow_up; only the wait ends, the sub-Agent continues, and the actual follow-up is attached immediately after this result in the same model context. Read and address that follow-up before deciding what to do next. Every assistant_text and final_answer string is returned completely, regardless of length. Each result also reports the latest known context usage and successful context-compaction count."
        }
        "Ask" => {
